...
void main() {
  ...
  DiaperEvent change(DiaperType type, {PoopSize? size, String? notes}) =>
      DiaperEvent(id: 'd', type: type, time: at, poopSize: size, notes: notes);

  group('a wet change cannot carry a size', () {
    test('the constructor drops it', () {
      final wet = change(DiaperType.wet, size: PoopSize.large);
      expect(wet.poopSize, isNull);
      expect(wet.hasPoop, isFalse);
    });
    ...
    test('wet + dirty counts as having stool', () {
      expect(change(DiaperType.both).hasPoop, isTrue);
      expect(change(DiaperType.dirty).hasPoop, isTrue);
      expect(change(DiaperType.wet).hasPoop, isFalse);
    });
  });
  ...
  group('how it reads on a row', () {
    test('size and notes join as one line', () {
      expect(
        DiaperFormat.details(
          change(DiaperType.dirty, size: PoopSize.large, notes: 'green'),
        ),
        'Large · green',
      );
    });
    ...
    test('either alone stands on its own', () {
      expect(
        DiaperFormat.details(change(DiaperType.dirty, size: PoopSize.small)),
        'Small',
      );
      expect(
    ...
    test('whitespace-only notes do not earn a separator', () {
      expect(
        DiaperFormat.details(
          change(DiaperType.dirty, size: PoopSize.medium, notes: '   '),
        ),
        'Medium',
      );
    });
